...
vec4 CalcSpotLight(Spot light, vec3 normal, vec3 viewDir, vec4 texColor)
{
    ...
    // Debug: Force intensity to 1.0 to test if angle calculation is the problem
    // intensity = 1.0; // Uncomment this line to test
    
    // Debug: Return red if we have any intensity
    // if (intensity > 0.0) return vec4(1.0, 0.0, 0.0, 1.0); // Uncomment to test
